...
cur.execute('CREATE TABLE List (Name, Number)')
...
def new_table(all):
    ...
    labels=[]
    for pair in all:
        labels.append(tk.Label(window, text=pair[0], relief=tk.SUNKEN, anchor=tk.W))
        labels.append(tk.Label(window, text=pair[1], relief=tk.SUNKEN, anchor=tk.W))
...
def add_user_command():
    if add_user_name_var.get() and add_user_number_var.get():
        execute(cur, 'INSERT INTO List VALUES (?,?)', add_user_name_var.get(), add_user_number_var.get())
...
def find_user_command():
    if find_user_var.get():
        new_table([(find_user_var.get(), number) for number in execute(cur, 'SELECT Number FROM List WHERE Name=?', find_user_var.get()).fetchall()])
        find_user_var.set('')

find_user_button=tk.Button(window, text='Vyhledat uživatele', command=find_user_command)
find_user_button.grid(row=1, column=2)

refresh_button=tk.Button(window, text='Zpět na seznam', command=find_user_command)
